...
$errors = [];
$vals   = [
    'titre' => '', 'slug' => '', 'auteur_id' => '', 'collection_id' => '',
    'isbn_papier' => '', 'isbn_numerique' => '', 'annee' => date('Y'),
    'nb_pages' => '', 'quatrieme' => '', 'extrait' => '',
    'prix_papier' => '', 'prix_numerique' => '',
    'disponible_papier' => 1, 'disponible_numerique' => 0,
    'statut' => 'brouillon', 'date_parution' => '', 'ordre' => 0,
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $vals['annee']               = (int)($_POST['annee']             ?? date('Y'));
    ...
    $chk = $pdo->prepare("SELECT id FROM livres WHERE slug = ?");
    ...
    if (!$errors) {
        $stmt = $pdo->prepare(
            "INSERT INTO livres (titre, slug, auteur_id, collection_id, isbn_papier, isbn_numerique,
        ...
        flash('success', 'Livre « ' . $vals['titre'] . ' » créé.');
...
<div class="admin-page-header">
  <h1 class="admin-page-title">Nouveau livre</h1>
...
<div class="row g-4">
  ...
  <div class="col-lg-8">
    ...
    <div class="admin-card mb-4">
      ...
      <div class="admin-card-body">
        <div class="admin-form-group mb-0">
          <label for="quatrieme" style="display:none">4ème de couv</label>
          <textarea id="quatrieme" name="quatrieme" rows="6" class="form-control"
                    placeholder="HTML basique accepté : &lt;p&gt;, &lt;strong&gt;, &lt;em&gt;"><?= e($vals['quatrieme']) ?></textarea>
        </div>
      </div>
    </div>
    ...
    <div class="admin-card mb-4">
      ...
      <div class="admin-card-body">
        <div class="admin-form-group mb-0">
          <label for="extrait" style="display:none">Extrait</label>
          <textarea id="extrait" name="extrait" rows="5" class="form-control"
                    placeholder="HTML basique accepté"><?= e($vals['extrait']) ?></textarea>
        </div>
      </div>
    </div>
  ...
  <div class="col-lg-4">
    ...
    <div class="admin-card mb-4">
      ...
      <div class="admin-card-body">
        ...
        <p class="mt-1 mb-0" style="font-size:.75rem;color:var(--admin-muted)">JPG, PNG, WEBP — max 400px largeur</p>
      </div>
    </div>
    ...
    <div class="admin-card mb-4">
      ...
      <div class="admin-card-body">
        <div class="row g-2">
          <div class="col-6">
            <div class="admin-form-group">
              <label for="prix_papier">Prix papier (€)</label>
              <input type="text" id="prix_papier" name="prix_papier" class="form-control"
                     value="<?= e($vals['prix_papier']) ?>" placeholder="18.00">
            </div>
          </div>
          <div class="col-6">
            <div class="admin-form-group">
              <label for="prix_numerique">Prix num. (€)</label>
              <input type="text" id="prix_numerique" name="prix_numerique" class="form-control"
                     value="<?= e($vals['prix_numerique']) ?>" placeholder="7.99">
            </div>
          </div>
        </div>
    ...
    <div class="d-flex gap-2">
      <button type="submit" class="admin-btn admin-btn-primary">Créer le livre</button>
