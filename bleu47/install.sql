...
-- Exécuter une seule fois sur la base nqmwgujbleu
-- ============================================================
...
CREATE TABLE IF NOT EXISTS `collections` (
  ...
  `description` TEXT,
  `couleur`     VARCHAR(7)    DEFAULT '#1a4a8a',
...
CREATE TABLE IF NOT EXISTS `livres` (
  ...
  `disponible_papier`    TINYINT(1)     DEFAULT 1,
  `disponible_numerique` TINYINT(1)     DEFAULT 0,
  `statut`               ENUM('publie','a_paraitre','epuise','brouillon') DEFAULT 'brouillon',
...
CREATE TABLE IF NOT EXISTS `actus` (
  ...
  `categorie`   ENUM('Actualité','Dédicace','Salon du livre','Parution') NOT NULL,
...
CREATE TABLE IF NOT EXISTS `revues_presse` (
  ...
  `livre_id`      INT           DEFAULT NULL,
  `type`          ENUM('Revue de presse','Critique') DEFAULT 'Revue de presse',
...
('Adrénaline',          'adrenaline', 'Polars, thrillers et romans noirs qui vous tiennent en haleine jusqu'à la dernière page.',     '#cc2200', 1),
('Fictions',            'fictions',   'Romans, récits et littérature contemporaine qui explorent la condition humaine.',                '#1a4a8a', 2),
('BD / Romans graphiques', 'bd',      'Bande dessinée et romans graphiques : quand le texte et l'image se répondent.',                 '#2d7a2d', 3);


INSERT INTO `auteurs` (`nom`, `prenom`, `slug`, `bio`, `statut`, `ordre`) VALUES
('Poux', 'Claude', 'claude-poux',
...
-- Romans de Claude Poux (auteur_id = 1)
INSERT INTO `livres`
