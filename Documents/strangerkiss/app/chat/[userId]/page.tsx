...
export default function ChatPage() {
      ...
      setMessages((prev) => {
        ...
        fetch(`/api/db/messages?from=${id}&to=${otherId}&since=${encodeURIComponent(since)}`)
          ...
          .then(({ messages: newMsgs }) => {
            if (newMsgs?.length > 0) {
              // D+�dupliquer par ID ��� +�vite le double affichage du selfie quand le poll
              // capture son `since` avant que pushMessage ait mis +� jour l'+�tat.
              setMessages((p) => {
  ...
  async function handleMeetingResponse(accept: boolean) {
    ...
    try {
      ...
      if (surveyDone !== "true") {
        ...
        console.log("[chat] ԣ� sk_survey_pending pos+� ��� dispatch sk:survey_check");
        // Notifier MapClient s'il est d+�j+� mont+� en m+�moire (router cache Next.js).
        ...
        console.log("[chat] survey d+�j+� effectu+� (sk_survey_done=true) ��� rien +� faire");
      }
    } catch (err) {
  ...
  function renderMessage(msg: Message) {
    ...
    // Avertissement de mod+�ration (envoy+� par le syst+�me +� l'utilisateur signal+�)
        ...
        <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
          ...
          <div className={`flex items-center gap-2 mt-1 ${isMe ? "justify-end" : "justify-between"}`}>
            ...
            {!isMe && (
              ...
              >
                {isTranslating ? <span className="animate-pulse">�Ŧ</span> : <>���� {translated ? "+�" : t("chat.translate")}</>}
              </button>
            )}
          </div>
        </div>
  ...
  // Chat d+�sactiv+� jusqu'au lancement (26 mai 2026) ��� sauf pour les b+�ta-testeurs
  ...
  if (isPrelaunch && isBeta === null) {
    return (
      <main className="flex flex-col w-full overflow-x-hidden items-center justify-center" style={{ height: "100dvh" }}>
        <div className="text-4xl animate-pulse">�Ŧ</div>
      </main>
    );
  }
      ...
      <main className="flex flex-col w-full overflow-x-hidden" style={{ height: "100dvh" }}>
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-5 px-6 text-center">
          ...
          <p className="text-sm text-white/50 max-w-xs leading-relaxed">
            {t("chat.comingSoonDesc", { date: LAUNCH_DATE.toLocaleDateString(bcp47, { day: "numeric", month: "long", year: "numeric" }) })}
          </p>
          <a
            href="/"
  ...
  return (
    <main className="flex flex-col w-full overflow-x-hidden" style={{ height: "100dvh" }}>
      ...
      <div className="flex-shrink-0 border-t border-white/5 bg-white/[0.02]">
        ...
        {/* Text input ��� min-w-0 sur l'input pour qu'il ne d+�passe pas en flex */}
